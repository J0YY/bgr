...
Latest poll/sync at 2026-06-10 10:30:56 BST: BGR identity `774713` completed
successfully with 391/400 successes, and matched-random identity `774715`
completed with 389/400 successes. Official identity `774711` was still marked
running, though the log tail had reached 393/400 successes. BGR and
matched-random hard-occlusion jobs `774714` and `774716` had just started on
`c2-g4-23`; official hard-occlusion `774712` remained dependency-pending.
Direct log tails showed the first BGR and matched-random hard-occlusion
...
Latest poll/sync at 2026-06-10 10:30:56 BST: official identity `774917` was
running at 20:40 elapsed, BGR identity `774920` at 12:09 elapsed, and
matched-random identity `774922` had started on `c2-g8-07` at 0:25 elapsed.
...
Latest poll/sync at 2026-06-10 12:51:17 BST showed the same scheduler state:
...
Latest status at 2026-06-10 10:30:56 BST: the A6000 adaptation route finally
started. Prep `774717` was running on `c2-g4-20` at 5:23 elapsed, official
identity `774724` was running on `c2-g4-20` at 4:59 elapsed with a log tail of
30/30 successes, and every BGR/random adaptation, merge, clean-eval, identity,
and hard-occlusion child job was still dependency-pending. The A40 fallback also
advanced: prep `774816` and BGR adaptation `774817` were completed, BGR merge
`774818` and matched-random adaptation `774820` were running on `c2-g8-07`,
and official identity `774846` was running at 25:48 elapsed with a log tail of
196/203 successes. A40 BGR clean eval `774819`, random merge/clean
...
Latest status at 2026-06-10 10:54:30 BST: no hard-occlusion route had a
complete compact `summary.csv`, so no fixed promotion gate could be run. The
0.65 transfer route still had all three hard-occlusion evals running:
official `774712` at 22:16, BGR `774714` at 24:11, and matched random
`774716` at 24:11; direct partial tails were official 73/101, BGR 73/104, and
matched random 74/105. The 0.80 transfer route remained identity-only:
official identity `774917` was running at 44:14, BGR identity `774920` at
35:43, matched-random identity `774922` at 18:41, and all three occlusion jobs
were dependency-pending; direct tails were official 366/373, BGR 280/288, and
active matched random 140/149. The A6000 adaptation route moved past BGR
training: prep `774717`, BGR adaptation `774718`, and BGR merge `774719`
completed, BGR clean `774720` and official identity `774724` were running,
matched-random adaptation `774721` was resource-pending, and BGR identity
`774726` was priority-pending; official identity tail was 166/173. The A40
fallback had prep, BGR adaptation/merge/clean, matched-random
adaptation/merge/clean, and official identity completed; official identity was
393/400, official occlusion `774847` had just started, replacement BGR
identity `775102` and matched-random identity `774850` were running, and
replacement BGR/matched-random occlusion jobs `775103`/`774851` were
dependency-pending. Direct A40 partials were BGR identity 55/57, official
occlusion 1/3, and matched-random identity 107/112. The latest readiness check
still reports `NOT_READY_FOR_90P_AAAI_CLAIM`; no paper claim changes.

Latest status at 2026-06-10 11:01:16 BST: no hard-occlusion route had a
complete compact `summary.csv`, so no fixed promotion gate could be run. The
0.65 transfer route still had official/BGR/matched-random hard-occlusion evals
running at 29:02/30:57/30:57; live log rsync dropped with code 12/255 and was
treated as pending, while direct partial tails were official 79/122, BGR
83/125, and matched random 82/125. The 0.80 transfer route moved past official
identity: official identity `774917` completed at 393/400, official occlusion
`774919` was running with direct tail 6/9, BGR identity was 349/357, and active
matched-random identity was 194/204; BGR and matched-random occlusion jobs
were dependency-pending. The A6000 adaptation route remained incomplete: BGR
clean `774720` and official identity `774724` were running, matched-random
adaptation `774721` was resource-pending, BGR identity `774726` was
priority-pending, and official identity tail was 211/218. The A40 fallback
remained incomplete: official occlusion `774847`, replacement BGR identity
`775102`, and matched-random identity `774850` were running; replacement BGR
occlusion `775103` and matched-random occlusion `774851` were dependency-
pending. Direct A40 partials were BGR identity 115/119, official occlusion
...
Latest status at 2026-06-10 11:09:06 BST: no hard-occlusion route had a
complete compact `summary.csv`, so no fixed promotion gate could be run. The
0.65 transfer route still had all three hard-occlusion evals running:
official `774712` at 36:52, BGR `774714` at 38:47, and matched random
`774716` at 38:47; direct partial tails were official 83/146, BGR 86/148, and
matched random 84/148, while synced available rows remained identity-only
at official 393/400, BGR 391/400, and matched random 389/400. The 0.80
transfer route had official and BGR identity complete at 393/400 and 391/400,
official occlusion `774919` running, BGR occlusion `774921` running,
matched-random identity `774922` running, and matched-random occlusion
`774923` dependency-pending; direct partial tails were official occlusion
45/58, BGR occlusion 5/7, and active matched-random identity 271/281. The
A6000 adaptation route had completed prep, BGR train/merge, and BGR clean eval;
matched-random adaptation `774721` and official identity `774724` were running,
BGR identity `774726` was resource-pending, and direct official identity tail
was 267/274. The A40 fallback had completed prep, both BGR/random
train/merge/clean chains, and official identity at 393/400; official occlusion
`774847`, replacement BGR identity `775102`, and matched-random identity
`774850` were running, while replacement BGR occlusion `775103` and
matched-random occlusion `774851` were dependency-pending. Direct A40 partials
were BGR identity 167/174, official occlusion 70/92, and matched-random
identity 220/227. The latest readiness check still reports
`NOT_READY_FOR_90P_AAAI_CLAIM`; no paper claim changes.

Latest status at 2026-06-10 11:12:56 BST: no hard-occlusion route had a
complete compact `summary.csv`, so no fixed promotion gate could be run. The
0.65 transfer route still had all three hard-occlusion evals running:
official `774712` at 40:42, BGR `774714` at 42:37, and matched random
`774716` at 42:37; direct partial tails were official 83/158, BGR 86/158, and
matched random 84/158, while synced available rows remained identity-only. The
0.80 transfer route had official/BGR identity complete, official occlusion
`774919`, BGR occlusion `774921`, and matched-random identity `774922`
running, and matched-random occlusion `774923` dependency-pending; direct
partial tails were official occlusion 55/78, BGR occlusion 28/32, and active
matched-random identity 317/327. The A6000 adaptation route had completed BGR
and matched-random training, BGR clean eval, and BGR identity had just started;
matched-random merge `774722` was resource-pending, official identity `774724`
was running with direct tail 302/309, and BGR identity tail was 6/6. The A40
fallback still had official occlusion `774847`, replacement BGR identity
`775102`, and matched-random identity `774850` running, with replacement BGR
occlusion `775103` and matched-random occlusion `774851` dependency-pending.
Direct A40 partials were BGR identity 204/211, official occlusion 73/107, and
matched-random identity 256/263. The latest readiness check still reports
`NOT_READY_FOR_90P_AAAI_CLAIM`; no paper claim changes.

Latest status at 2026-06-10 11:17:49 BST: no hard-occlusion route had a
complete compact `summary.csv`, so no fixed promotion gate could be run. The
0.65 transfer route still had all three hard-occlusion evals running at
45:35/47:30/47:30 for official/BGR/matched-random; the live-log rsync dropped
with code 12/255, but direct tails showed official 110/188, BGR 111/187, and
matched random 109/187. The 0.80 transfer route still had official occlusion
`774919` and BGR occlusion `774921` running, but matched-random identity
`774922` returned to pending on `Priority` after reaching 317/327 in its log,
with estimated start 2026-06-11 22:02:14 BST; matched-random occlusion `774923`
remained dependency-pending. Direct 0.80 occlusion tails were official 68/103
and BGR 47/60. The A6000 adaptation route had matched-random merge `774722`,
official identity `774724`, and BGR identity `774726` running; direct tails
were official identity 347/354 and BGR identity 33/33, with downstream
occlusion and matched-random evals still dependency-pending. The A40 fallback
still had official occlusion `774847` running, while replacement BGR identity
`775102` and matched-random identity `774850` returned to pending after
partial logs of 204/211 and 256/263; official occlusion reached 78/128.
Replacement BGR/random occlusions remained dependency-pending, and the original
failed BGR child remains ignored. The latest readiness check still reports
`NOT_READY_FOR_90P_AAAI_CLAIM`; no paper claim changes.

Latest status at 2026-06-10 11:25:28 BST: no hard-occlusion route had a
complete compact `summary.csv`, so no fixed promotion gate could be run. The
0.65 transfer route still had all three hard-occlusion evals running at
52:15/54:10/54:10 for official/BGR/matched-random; the synced
`summary_available.csv` remained identity-only, and direct occlusion tails were
official 133/222, BGR 134/220, and matched random 132/221. The 0.80 transfer
route had official occlusion `774919` running at 25:29 and BGR occlusion
`774921` running at 16:58, with matched-random identity `774922`
priority-pending for an estimated 2026-06-11 10:59:00 BST start and
matched-random occlusion `774923` dependency-pending; direct occlusion tails
were official 74/135 and BGR 72/103, while matched-random identity remained
partial/stale at 317/327. The A6000 adaptation route had prep, BGR
train/merge/clean, random train/merge, and official identity effectively
completed; random clean `774723` and BGR identity `774726` were running, random
identity `774728` was resource-pending, and all occlusion evals were
dependency-pending. Direct tails were official identity complete at 393/400 and
BGR identity 87/91. The A40 fallback still had official occlusion `774847`
running, replacement BGR identity `775102` and matched-random identity `774850`
priority-pending after partial logs of 204/211 and 256/263, and replacement
BGR/matched-random occlusions dependency-pending; official occlusion reached
82/158. The latest readiness check still reports
`NOT_READY_FOR_90P_AAAI_CLAIM`; no paper claim changes.

Latest status at 2026-06-10 11:30:01 BST: no hard-occlusion route had a
complete compact `summary.csv`, so no fixed promotion gate could be run. The
0.65 transfer route still had all three hard-occlusion evals running at
56:42/58:37/58:37 for official/BGR/matched-random; direct occlusion tails were
official 145/241, BGR 147/238, and matched random 143/238. The 0.80 transfer
route had official occlusion `774919` running at 30:00 and BGR occlusion
`774921` running at 21:29, while matched-random identity `774922` remained
priority-pending with estimated start 2026-06-11 11:25:00 BST and
matched-random occlusion `774923` remained dependency-pending; direct partials
were official 78/152 and BGR 77/123, with no matched-random occlusion row yet.
The A6000 adaptation route had official identity complete at 393/400, BGR
identity `774726` running at 17:47, matched-random identity `774728` running at
3:46, random clean `774723` running, official occlusion `774725`
priority-pending, and BGR/random occlusions dependency-pending; direct tails
were BGR identity 113/120 and matched random identity 23/23. The A40 fallback
still had only official identity in `summary_available.csv`; official occlusion
`774847` was running, replacement BGR identity `775102` and matched-random
identity `774850` were pending, replacement BGR/random occlusions were
dependency-pending, and original failed BGR chain `774848`/`774849` remains
ignored. The latest readiness check still reports
`NOT_READY_FOR_90P_AAAI_CLAIM`; no paper claim changes.

Latest status at 2026-06-10 11:34:03 BST: no hard-occlusion route had a
complete compact `summary.csv`, so no fixed promotion gate could be run. The
0.65 transfer route still had all three hard-occlusion evals running at
1:00:47/1:02:42/1:02:42 for official/BGR/matched-random; the synced
`summary_available.csv` remained identity-only. The 0.80 transfer route had
official occlusion `774919` running at 34:02 and BGR occlusion `774921`
running at 25:31, but matched-random identity `774922` remained
priority-pending for an estimated 2026-06-11 11:25:00 BST start and
matched-random occlusion `774923` remained dependency-pending. The A6000
adaptation route was still moving: random clean `774723`, BGR identity
`774726`, and matched-random identity `774728` were running; official occlusion
`774725` was priority-pending; BGR/random occlusions were dependency-pending.
The A40 fallback still had official occlusion `774847` running, replacement
BGR identity `775102` priority-pending, matched-random identity `774850`
resource-pending, replacement BGR/random occlusions dependency-pending, and the
...
Latest status at 2026-06-10 11:40:24 BST: no hard-occlusion route had a
complete compact `summary.csv`, so no fixed promotion gate could be run. The
0.65 transfer route still had all three hard-occlusion evals running at
1:07:04/1:08:59/1:08:59 for official/BGR/matched-random; the synced
`summary_available.csv` remained identity-only. The 0.80 transfer route had
official occlusion `774919` running at 40:20 and BGR occlusion `774921`
running at 31:49, while matched-random identity `774922` remained
priority-pending with estimated start 2026-06-11 11:36:00 BST and
matched-random occlusion `774923` remained dependency-pending. The A6000
adaptation route was still progressing: official occlusion `774725`, BGR
identity `774726`, and matched-random identity `774728` were running, while
BGR/random occlusions were dependency-pending. The A40 fallback remained
scheduler-limited with official occlusion running, replacement BGR identity and
matched-random identity pending, replacement BGR/random occlusions
dependency-pending, and the original failed BGR chain ignored. The latest
readiness check still reports `NOT_READY_FOR_90P_AAAI_CLAIM`; no paper claim
changes.

Latest status at 2026-06-10 11:43:41 BST: no hard-occlusion route had a
complete compact `summary.csv`, so no fixed promotion gate could be run. The
0.65 transfer route still had all three hard-occlusion evals running at
1:10:18/1:12:13/1:12:13 for official/BGR/matched-random; the synced
`summary_available.csv` remained identity-only. The 0.80 transfer route had
official occlusion `774919` running at 43:36 and BGR occlusion `774921`
running at 35:05, while matched-random identity `774922` remained
priority-pending and matched-random occlusion `774923` remained
dependency-pending. The A6000 adaptation route was still progressing but
incomplete: official occlusion `774725`, BGR identity `774726`, and
matched-random identity `774728` were running; BGR/random occlusions remained
dependency-pending. The A40 fallback remained scheduler-limited with official
occlusion running, replacement BGR identity and matched-random identity
pending, replacement BGR/random occlusions dependency-pending, and the original
failed BGR chain ignored. The latest readiness check still reports
`NOT_READY_FOR_90P_AAAI_CLAIM`; no paper claim changes.

Latest status at 2026-06-10 11:48:06 BST: all four hard-occlusion routes still
...
evals still running at 1:13:46/1:15:41/1:15:41, and the synced
`summary_available.csv` remained identity-only with official 393/400, BGR
391/400, and matched random 389/400. The 0.80 transfer route had official and
BGR occlusion running at 47:05 and 38:34, while matched-random identity was
priority-pending until 2026-06-11 11:36:00 BST and matched-random occlusion
was dependency-pending. The A6000 adaptation route was past prep, both
training/merge chains, and both clean evals, but official occlusion, BGR
identity, and matched-random identity were still running; BGR/random occlusions
were dependency-pending. The A40 fallback had official occlusion running,
replacement BGR identity priority-pending until 2026-06-11 22:02:14 BST,
matched-random identity resource-pending until 2026-06-10 23:36:39 BST, and
replacement BGR/random occlusions dependency-pending, with the original failed
BGR chain ignored. The latest readiness check still reports
`NOT_READY_FOR_90P_AAAI_CLAIM`; no paper claim changes.

Latest status at 2026-06-10 11:51:33 BST: all four hard-occlusion routes still
lacked complete compact `summary.csv` files, so no fixed promotion gate could
be run. The 0.65 transfer route had official/BGR/matched-random hard-occlusion
evals still running at 1:18:10/1:20:05/1:20:05, with only identity rows in the
synced `summary_available.csv`. The 0.80 transfer route had official and BGR
occlusion running at 51:28 and 42:57, while matched-random identity was still
priority-pending until 2026-06-11 11:36:00 BST and matched-random occlusion was
dependency-pending. The A6000 adaptation route remained incomplete with
official occlusion, BGR identity, and matched-random identity running, while
BGR/random occlusions were dependency-pending. The A40 fallback had official
occlusion running, replacement BGR identity priority-pending until
2026-06-11 22:02:14 BST, matched-random identity resource-pending until
2026-06-10 23:36:39 BST, and replacement BGR/random occlusions
dependency-pending, with the original failed BGR chain ignored. The latest
readiness check still reports `NOT_READY_FOR_90P_AAAI_CLAIM`; no paper claim
changes. The scorecard still identifies these hard-occlusion learned-policy
jobs as the current acceptance-moving work and warns against adding more
...
Latest status at 2026-06-10 12:02:47 BST: the completed 0.65 transfer route
...
running at 1:02:40 and 54:09, with matched-random identity priority-pending
until 2026-06-11 11:36:00 BST and matched-random occlusion dependency-pending.
The A6000 adaptation route remains the main active learned-policy candidate:
official occlusion, BGR identity, and matched-random identity are running, while
BGR/random occlusions are dependency-pending. The A40 fallback remains
scheduler-limited with official occlusion priority-pending, replacement BGR
...
Latest status at 2026-06-10 12:08:40 BST: the completed 0.65 transfer route
remains negative/non-promotable, and the remaining active hard-occlusion routes
are still incomplete. In the 0.80 transfer route, official occlusion completed
successfully, BGR occlusion is still running, matched-random identity is
resource-pending with estimated start 2026-06-10 23:36:39 BST, and
matched-random occlusion is dependency-pending. The A6000 adaptation route
remains the main active learned-policy candidate: official occlusion, BGR
identity, and matched-random identity are running, while BGR/random occlusions
are dependency-pending. The A40 fallback has matched-random identity running,
but official occlusion and replacement BGR identity are still priority-pending,
...
Latest status at 2026-06-10 12:51:17 BST: no active hard-occlusion route is
gateable. The 0.80 identity-anchored prep `776029` is healthy and now
generating TFDS examples, but all train/eval children remain dependency-pending.
The 0.80 transfer route still lacks matched-random rows; random identity
`774922` is running with direct tail 305/315, and random occlusion `774923`
is dependency-pending. The A6000 0.65 adaptation route remains incomplete and
...
at 2026-06-11 11:42:26 BST showed official `783104` and BGR `783105`
completed with exit `0:0`, while matched random `783106` was still pending
after a delayed partial log. The incomplete summary has BGR 293/400, official
295/400, and matched random 279/378, so BGR already trails official on the
fixed 400-episode readout and trails matched-random by success rate. Because
the scout was already non-promotable, `783106` was cancelled to avoid spending
more GPU time on a dead comparator. Treat it as a severity-window diagnostic
...
`783315`; latest guarded advance at 2026-06-11 12:55:31 BST ran
...
2026-06-11 14:21:02 BST for official, 2026-06-11 18:01:36 BST for BGR, and
2026-06-11 18:51:35 BST for matched random. A direct scheduler check showed no
dependencies or feature constraints, priority 611 for all three jobs, and only
explicit exclusion `c2-g4-21`, which is down; leave the jobs untouched unless a
new scheduler problem appears. The remote summary and log directory were still
missing at the 12:55:31 BST poll. This is only
...
the post-render family filter, so prep `782649` exited `1:0`; the dependent
BGR/random adaptation and eval jobs were cancelled. The renderer now supports
pre-selection family filtering with `--include-family`, and the occlusion
bottleneck queue script passes `--include-family occlusion` to both BGR and
matched-random perturb renders. Because the remote manifest has 1,600 BGR
occlusion rows but only 512 matched-random occlusion rows, the corrected active
route uses a fair matched cap of 512 and repeats both occlusion sources twice:
`PREP_TAG=p512unique_occonly_hardocc080_router_randfix_prereg`,
`OCCLUSION_CAP=512`, `OCCLUSION_REPEAT=2`, `INCLUDE_CLEAN_ANCHORS=0`,
`OCCLUSION_FRACTION_OVERRIDE=0.80`, `PROXIMAL_ANCHOR_L2=0`,
`ADAPT_STEPS=300`, and `LR=5e-7`. Submitted jobs are prep `782671`, BGR
train/merge/clean-eval `782672`/`782673`/`782674`, matched-random
train/merge/clean-eval `782675`/`782676`/`782677`, and official/BGR/random
hard-occlusion evals `782679`/`782680`/`782681`. This remains router-premise
evidence only; do not add it to the manuscript unless BGR beats both official
...
JOB_IDS=782671,782672,782673,782674,782675,782676,782677,782679,782680,782681 \
DETAIL_JOB_IDS=782671,782672,782673,782675,782676,782679,782680,782681 \
ROUTE_LABEL='Hard-occlusion 0.80 occlusion-only router-trained OpenVLA-OFT randfix premise' \
GATE_PERTURBATIONS=occlusion \
scripts/sync_openvla_oft_occlusion_bottleneck_results.sh --poll --sync --no-check
```
