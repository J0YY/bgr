# OpenVLA-OFT BGR-Boundary Balanced-2048 Step-10 Fine-Tune

This artifact records a bounded OpenVLA-OFT LoRA fine-tuning run on the
BGR-boundary 2,048-step TFDS export. The run verifies that the larger
BGR-selected dataset can drive the unmodified OpenVLA-OFT training script past
model loading, RLDS construction, ten optimizer steps, and checkpoint writing.
...
- Slurm allocated job 763254 on `c2-g4-22`.
- OpenVLA 7B loaded from the Hugging Face cache.
- LoRA rank 8, proprio projector, and L1 action head were initialized.
- OpenVLA-OFT loaded the BGR `libero_goal_no_noops` TFDS dataset with 2,048
  transitions and 32 trajectories.
...
```text
/work/joy/bgr/runs/openvla_oft_finetune_bgr_balanced2048_step10_v1/openvla-7b+libero_goal_no_noops+b1+lr-0.0005+lora-r8+dropout-0.0--bgr-balanced2048-step10
```
