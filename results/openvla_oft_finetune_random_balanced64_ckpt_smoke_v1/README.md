# OpenVLA-OFT Random-Balanced-64 Fine-Tune Checkpoint Smoke

This run is a one-step OpenVLA-OFT LoRA fine-tuning smoke on the
random-balanced 64-step TFDS dataset. It mirrors the BGR balanced64 checkpoint
smoke and verifies that the matched baseline dataset can drive the unmodified
OpenVLA-OFT training script through model loading, RLDS loading,
forward/backward optimization, and checkpoint writing.
...
- OpenVLA-OFT loaded the random-balanced TFDS dataset and reused its
  64-transition, 4-trajectory statistics.
...
```text
/work/joy/bgr/runs/openvla_oft_finetune_random_balanced64_ckpt_smoke_v1/openvla-7b+libero_goal_no_noops+b1+lr-0.0005+lora-r8+dropout-0.0--random-balanced64-ckpt-smoke
```
