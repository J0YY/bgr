...
\subsection{Planned Procedural RL Study}

The next stage instantiates replayable states in procedural grid environments with resettable seeds and checkpointed states. Perturbations include start-pose offsets with reachability filters. Baselines include uniform replay, PLR-style loss priority, failure-only replay, fixed-radius training, random-radius perturbation, and BGR ablations. An initial tabular grid-recovery benchmark is implemented, but the first run is not a positive result: broad replay saturates the environment after clean pretraining, while the current BGR sampler remains too narrow for saturated states. This motivates hardening the procedural benchmark with held-out layouts, longer perturbation horizons, and a saturated-state coverage fallback before using it as main evidence.
