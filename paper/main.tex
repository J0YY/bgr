...
The following local calculation is a design rationale for the radius sampler and for the radius-level ablation, not a convergence result, global robustness theorem, or margin-expansion guarantee.
...
BGR converts recovery-margin measurement into a replay curriculum. The evidence supports a mechanism claim: with reset access, feasible perturbations, and directly trainable recovery curves, sampling near the estimated success--failure boundary improves margin expansion over uniform, fixed-radius, failure-only, and loss-proxy replay. The radius-level ablation is the clearest novelty evidence: hard-state priority alone loses the grid-margin effect when radii are drawn uniformly. Synthetic and grid-margin studies establish the mechanism; suffix results show scoped transfer to manipulation-style replay states. The current learned-policy evidence remains an audit, not the final robotics result.
