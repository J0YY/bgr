...
TAXI_WALLS = {
    (row, 1, row, 2) for row in (0, 1)
} | {
    (row, 2, row, 1) for row in (0, 1)
} | {
    item
    for row in (3, 4)
    for item in (
...
def decode(state: int) -> tuple[int, int, int, int]:
    destination = state % 4
    state //= 4
    passenger = state % 5
    state //= 5
...
def taxi_step(state: int, action: int) -> tuple[int, float, bool]:
    row, col, passenger, destination = decode(state)
    next_row, next_col = row, col
    reward = -1.0
    done = False

    if action == 0:
        next_row = min(row + 1, 4)
    elif action == 1:
        next_row = max(row - 1, 0)
    elif action == 2:
        next_col = min(col + 1, 4)
    elif action == 3:
        next_col = max(col - 1, 0)
    elif action == 4:
        if passenger < 4 and (row, col) == LOCATIONS[passenger]:
            passenger = 4
        else:
            reward = -10.0
        return encode(row, col, passenger, destination), reward, False
    elif action == 5:
        if passenger == 4 and (row, col) == LOCATIONS[destination]:
            passenger = destination
            reward = 20.0
            done = True
        elif passenger == 4 and (row, col) in LOCATIONS:
            passenger = LOCATIONS.index((row, col))
            reward = -10.0
        else:
            reward = -10.0
        return encode(row, col, passenger, destination), reward, done
    else:
        raise ValueError(f"unknown Taxi action: {action}")

    if (row, col, next_row, next_col) in TAXI_WALLS:
        next_row, next_col = row, col
    return encode(next_row, next_col, passenger, destination), reward, done


def value_iteration(discount: float, iterations: int = 900) -> np.ndarray:
    q = np.zeros((500, 6), dtype=float)
    for _ in range(iterations):
        next_q = q.copy()
        for state in range(500):
            for action in range(6):
                next_state, reward, done = taxi_step(state, action)
                next_q[state, action] = reward if done else reward + discount * float(np.max(q[next_state]))
        if float(np.max(np.abs(next_q - q))) < 1e-9:
            return next_q
        q = next_q
    return q


class TaxiRecoveryProbe:
    def __init__(
        self,
        *,
        ...
        self.rng = np.random.default_rng(seed + 62_000)
        ...
        self.q = self.q + self.rng.normal(0.0, float(q_init_noise), size=self.q.shape)
        self.states = self._select_replay_states(int(replay_state_count))
        self.perturbation_cache = self._build_perturbation_cache()

    def _select_replay_states(self, count: int) -> list[TaxiReplayState]:
        candidates: list[tuple[int, TaxiReplayState]] = []
        for passenger in range(4):
            for destination in range(4):
                if passenger == destination:
                    continue
                passenger_row, passenger_col = LOCATIONS[passenger]
                dest_row, dest_col = LOCATIONS[destination]
                for row in range(5):
                    for col in range(5):
                        distance = min(
                            abs(row - passenger_row) + abs(col - passenger_col),
                            abs(row - dest_row) + abs(col - dest_col),
                        )
                        if 1 <= distance <= self.max_radius:
                            replay = TaxiReplayState(
                                state=encode(row, col, passenger, destination),
                                row=row,
                                col=col,
                                passenger=passenger,
                                destination=destination,
                            )
                            candidates.append((distance, replay))
        candidates.sort(key=lambda item: (item[0], item[1].passenger, item[1].destination, item[1].row, item[1].col))
    ...
    def _build_perturbation_cache(self) -> tuple[tuple[tuple[int, ...], ...], ...]:
        ...
        for replay in self.states:
            ...
            for radius in range(self.max_radius + 1):
                states = tuple(
                    encode(row, col, replay.passenger, replay.destination)
                    for row in range(5)
                    for col in range(5)
                    if abs(row - replay.row) + abs(col - replay.col) == radius
                )
                radii.append(states or (replay.state,))
...
def run_method(args: argparse.Namespace, optimal_q: np.ndarray, method: str, seed: int) -> TaxiProbeResult:
    rng = np.random.default_rng(seed + 71_000)
    bench = TaxiRecoveryProbe(
        optimal_q=optimal_q,
        seed=seed,
...
def init_records(bench: TaxiRecoveryProbe, rng: np.random.Generator, args: argparse.Namespace) -> list[LevelRecord]:
    records: list[LevelRecord] = []
    for replay_idx, replay in enumerate(bench.states):
        record = LevelRecord(
            id=f"taxi_{replay.state}",
            domain="gym_taxi_v3_recovery",
            task_id="Taxi-v3",
            clean_success_hat=bench.success_prob(replay_idx, 0.0),
            feasibility_hat=1.0,
            perturbation_family="taxi_position_manhattan_restart",
...
def evaluate(bench: TaxiRecoveryProbe, args: argparse.Namespace) -> dict[str, float]:
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--out", default="runs/taxi_recovery_probe")
    ...
    parser.add_argument("--replay-states", type=int, default=60)
    ...
    parser.add_argument("--q-init-blend", type=float, default=0.12)
