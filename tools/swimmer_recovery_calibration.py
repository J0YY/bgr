#!/usr/bin/env python3
"""Pre-method recovery calibration for Gymnasium MuJoCo Swimmer-v5."""
...
def sinusoidal_action(step: int, *, amplitude: float, frequency: float, action_dim: int) -> np.ndarray:
    phases = np.arange(action_dim, dtype=float) * (np.pi / 2.0)
    return np.clip(float(amplitude) * np.sin(float(frequency) * int(step) + phases), -1.0, 1.0)


def checkpoint(env, *, seed: int, burn_in: int, amplitude: float, frequency: float) -> SwimmerCheckpoint:
    env.reset(seed=int(seed))
    action_dim = int(np.prod(env.action_space.shape))
    phase = 0
    for phase in range(int(burn_in)):
        _obs, _reward, terminated, truncated, _info = env.step(
            sinusoidal_action(phase, amplitude=amplitude, frequency=frequency, action_dim=action_dim)
        )
        if terminated or truncated:
            break
    return SwimmerCheckpoint(
        seed=int(seed),
...
def perturb_state(
    ...
    *,
    ...
    rng = np.random.default_rng(613_000 + 10_003 * int(item.seed) + 997 * int(trial))
    qpos = np.array(item.qpos, dtype=float)
    qvel = np.array(item.qvel, dtype=float)
    pos_direction = rng.normal(0.0, 1.0, size=qpos.shape)
    vel_direction = rng.normal(0.0, 1.0, size=qvel.shape)
    pos_direction[0] = 0.0
    pos_direction /= float(np.linalg.norm(pos_direction) + 1e-12)
    vel_direction /= float(np.linalg.norm(vel_direction) + 1e-12)
    pos_scales = np.full_like(qpos, float(position_scale), dtype=float)
    pos_scales[0] = 0.0
    return (
        qpos + float(radius) * pos_scales * pos_direction,
        qvel + float(radius) * float(velocity_scale) * vel_direction,
    )
...
def rollout(
    ...
    *,
    ...
    for step in range(int(args.horizon)):
        _obs, _reward, terminated, truncated, _info = env.step(
            sinusoidal_action(
                ...
                frequency=args.controller_frequency,
    ...
    return SwimmerCalibrationRow(
        seed=int(item.seed),
        radius=float(radius),
        trial=int(trial),
        success=int(progress >= float(args.min_progress)),
...
def summarize(rows: list[SwimmerCalibrationRow], radii: np.ndarray, *, alpha: float) -> dict[str, object]:
    curve = []
    progress_curve = []
    for radius in radii:
...
def run(args: argparse.Namespace) -> None:
    ...
    rows: list[SwimmerCalibrationRow] = []
    for seed in range(int(args.seeds)):
        ...
        for radius in radii:
            for trial in range(int(args.trials)):
                rows.append(rollout(env, item, radius=float(radius), trial=trial, args=args))
        env.close()
    ...
    summary.update(
        {
            ...
            "controller_frequency": float(args.controller_frequency),
        }
    )
...
def main() -> None:
    ...
    parser.add_argument("--env-id", default="Swimmer-v5")
    parser.add_argument("--seeds", type=int, default=12)
    parser.add_argument("--radii", default="0,0.25,0.5,0.75,1.0,1.25,1.5,2.0")
    ...
    parser.add_argument("--velocity-scale", type=float, default=0.40)
    parser.add_argument("--controller-amplitude", type=float, default=1.0)
    parser.add_argument("--controller-frequency", type=float, default=0.40)
