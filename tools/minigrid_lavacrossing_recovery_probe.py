...
class OfficialMiniGridLavaCrossingProbe:
    def __init__(
        self,
        *,
        seed: int,
    ...
    def _free_cells(self) -> tuple[tuple[int, int], ...]:
        ...
        for x in range(self.width):
            for y in range(self.height):
                ...
                if obj is None or obj.type == "goal":
                    cells.append((x, y))
        return tuple(cells)
    ...
    def _step_from(self, state_idx: int, action_idx: int) -> tuple[int, float, bool]:
        ...
        if bool(terminated):
            return state_idx, 1.0 if reward > 0.0 else 0.0, True
        if bool(truncated):
    ...
    def _select_replay_states(self, count: int) -> list[MiniGridReplayState]:
        path_cells = self._shortest_path_cells()
        ...
        for x, y in self.free_cells:
            if (x, y) in self.goal_cells:
                continue
            nearest_path = min(abs(x - px) + abs(y - py) for px, py in path_cells)
            nearest_hazard = min(abs(x - hx) + abs(y - hy) for hx, hy in hazards) if hazards else self.width + self.height
            if nearest_path <= 1 and nearest_hazard <= 2:
                for direction in DIRS:
                    dist = distances.get((x, y), self.width * self.height)
                    candidates.append((dist, nearest_hazard, direction, MiniGridReplayState(x, y, direction)))
        if not candidates:
    ...
    def _hazard_cells(self) -> tuple[tuple[int, int], ...]:
        cells: list[tuple[int, int]] = []
        for x in range(self.width):
            for y in range(self.height):
                obj = self.grid.get(x, y)
                if obj is not None and obj.type == "lava":
                    cells.append((x, y))
        return tuple(cells)

    def _shortest_path_cells(self) -> tuple[tuple[int, int], ...]:
        start = tuple(int(v) for v in self.unwrapped.agent_pos)
        goal = next(iter(self.goal_cells))
        frontier = [start]
        ...
        while frontier:
            ...
            for dx, dy in NEIGHBORS:
                nx, ny = x + dx, y + dy
                if (nx, ny) in parent or (nx, ny) not in self.free_cells:
                    continue
                parent[(nx, ny)] = (x, y)
                frontier.append((nx, ny))
        if goal not in parent:
            return tuple(cell for cell in self.free_cells if cell not in self.goal_cells)
        cells: list[tuple[int, int]] = []
    ...
    def _distances_from(self, starts: list[tuple[int, int]]) -> dict[tuple[int, int], int]:
    ...
    def _bottleneck_cells(self) -> tuple[tuple[int, int], ...]:
        ...
        for x, y in self.free_cells:
            ...
            for hx, hy in hazards:
                hazard_neighbors += int(abs(x - hx) + abs(y - hy) == 1)
            if hazard_neighbors:
                cells.append((x, y))
        return tuple(cells)
...
def run_method(args: argparse.Namespace, method: str, seed: int) -> MiniGridProbeResult:
    ...
    bench = OfficialMiniGridLavaCrossingProbe(
        seed=seed,
        env_id=args.env_id,
...
def init_records(bench: OfficialMiniGridLavaCrossingProbe, rng: np.random.Generator, args: argparse.Namespace) -> list[LevelRecord]:
    records: list[LevelRecord] = []
    for replay_idx, replay in enumerate(bench.states):
        record = LevelRecord(
            id=f"minigrid_lavacrossing_{replay.x}_{replay.y}_{replay.direction}",
            domain="minigrid_lavacrossing_recovery",
            task_id=bench.env_id,
            clean_success_hat=bench.success_prob(replay_idx, 0.0),
            feasibility_hat=1.0,
            perturbation_family="official_minigrid_lava_position_restart",
...
def evaluate(bench: OfficialMiniGridLavaCrossingProbe, args: argparse.Namespace) -> dict[str, float]:
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Run an internal official MiniGrid-LavaCrossing recovery replay diagnostic.")
    parser.add_argument("--out", default="runs/minigrid_lavacrossing_recovery_probe")
    parser.add_argument(
        "--env-id",
