#!/usr/bin/env python3
"""Run a fixed MinAtar Freeway recovery replay screen.

This external-package screen is authorized by the fixed 20-seed Freeway
calibration. It uses MinAtar's package-owned Freeway dynamics, exact
seed/checkpoint reconstruction, a fixed safe-up teacher, and downward
player-row perturbations.
"""
...
from tools.minatar_freeway_recovery_calibration import checkpoint, controller_action, perturb_player, row_danger
...
def package_versions() -> dict[str, str]:
    try:
        ...
        raise SystemExit(
            "MinAtar Freeway recovery probe requires MinAtar in an isolated "
...
class FreewayRecoveryProbe:
    def __init__(self, *, seed: int, args: argparse.Namespace) -> None:
        ...
        self.args = args
        self.rng = np.random.default_rng(seed + 914_000)
    ...
    def restore_start(self, replay_idx: int, sigma: float) -> None:
        ...
        perturb_player(self.env, sigma=float(sigma))
    ...
    def rollout(self, replay_idx: int, sigma: float, *, train: bool) -> bool:
        ...
        for _step in range(1, int(self.args.horizon) + 1):
            ...
            if terminal:
                break
        return (not bool(self.env.env.terminal)) and reward >= 1

    def features(self) -> np.ndarray:
        return freeway_features(self.env)


def freeway_features(env: Any) -> np.ndarray:
    game = env.env
    pos = int(game.pos)
    up_row = max(0, pos - 1)
    down_row = min(9, pos + 1)
    car_rows = np.zeros(10, dtype=float)
    car_dx_min = np.ones(10, dtype=float)
    car_timer_min = np.ones(10, dtype=float)
    for car_x, car_y, timer, speed in game.cars:
        row = int(car_y)
        car_rows[row] = 1.0
        car_dx_min[row] = min(car_dx_min[row], abs(int(car_x) - 4) / 9.0)
        car_timer_min[row] = min(car_timer_min[row], min(int(timer), 5) / 5.0)
        next_x = int(car_x)
        if int(timer) == 0:
            next_x += 1 if int(speed) > 0 else -1
            if next_x < 0:
                next_x = 9
            elif next_x > 9:
                next_x = 0
            car_dx_min[row] = min(car_dx_min[row], abs(next_x - 4) / 9.0)
    local_rows = [up_row, pos, down_row]
    local_danger = [float(row_danger(game, row)) for row in local_rows]
    local_dx = [float(car_dx_min[row]) for row in local_rows]
    local_timer = [float(car_timer_min[row]) for row in local_rows]
    return np.array(
        [
            pos / 9.0,
            (9 - pos) / 9.0,
            int(game.move_timer) / 5.0,
            int(game.terminate_timer) / 2500.0,
            *local_danger,
            *local_dx,
            *local_timer,
            float(not local_danger[0]),
            float(not local_danger[1]),
            float(not local_danger[2]),
            float(pos == 0),
            float(pos <= 3),
            float(pos >= 7),
            float(np.mean(car_rows[1:9])),
            float(np.min(car_dx_min[1:9])),
            float(np.mean(car_timer_min[1:9])),
            1.0,
        ],
        dtype=float,
    )
...
def run_method(args: argparse.Namespace, method: str, seed: int) -> FreewayProbeResult:
    rng = np.random.default_rng(seed + 915_000)
    bench = FreewayRecoveryProbe(seed=seed, args=args)
    try:
...
def init_records(bench: FreewayRecoveryProbe, args: argparse.Namespace) -> list[LevelRecord]:
    records: list[LevelRecord] = []
    for replay_idx, replay_seed in enumerate(bench.replay_seeds):
        record = LevelRecord(
            id=f"minatar_freeway_seed_{replay_seed}",
            domain="minatar_freeway_recovery",
            task_id="MinAtar-Freeway",
            clean_success_hat=bench.success_prob(replay_idx, 0.0),
            feasibility_hat=1.0,
            perturbation_family="downward_player_row_displacement",
...
def evaluate(bench: FreewayRecoveryProbe, args: argparse.Namespace) -> dict[str, float]:
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--out", default="runs/minatar_freeway_recovery_probe")
    ...
    parser.add_argument("--target-radius", type=float, default=1.8666666666666667)
    parser.add_argument("--radius-bandwidth", type=float, default=1.5)
    parser.add_argument("--fixed-radius", type=float, default=2.0)
    parser.add_argument("--baseline-candidates", type=int, default=8)
    parser.add_argument("--initial-probes", type=float, nargs="+", default=[0.0, 1.0, 2.0, 4.0, 6.0, 9.0])
