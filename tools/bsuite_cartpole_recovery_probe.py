#!/usr/bin/env python3
"""Run a fixed bsuite Cartpole recovery replay screen.

This optional external-package screen uses bsuite's package-owned Cartpole
dynamics through exact CartpoleState restarts. The perturbation family moves
near-upright replay states toward larger cart, pole-angle, and velocity errors
while preserving feasibility under bsuite's height and cart-position limits.
"""
...
def package_versions() -> dict[str, str]:
    try:
        ...
        raise SystemExit(
            "bsuite Cartpole recovery probe requires bsuite in an isolated environment, "
...
class CartpoleRecoveryProbe:
    ...
    def _select_replay_states(self, count: int) -> list[CartpoleReplayState]:
        angles = np.linspace(-0.46, 0.46, 9)
        angle_velocities = np.linspace(-1.35, 1.35, 9)
        positions = np.linspace(-1.15, 1.15, 5)
        cart_velocities = np.linspace(-0.45, 0.45, 3)
        candidates = [
            ...
            if abs(theta) + 0.06 * abs(theta_dot) + 0.02 * abs(x) >= 0.08
            and np.cos(theta) > self.height_threshold
            and abs(x) < self.x_threshold
        ]
        if not candidates:
            raise ValueError("no feasible bsuite Cartpole replay states selected")
        candidates.sort(
            key=lambda item: (
    ...
    def _clip_state(self, state: np.ndarray) -> CartpoleReplayState:
        return CartpoleReplayState(
            ...
            theta_dot=float(np.clip(state[3], -3.2, 3.2)),
        )
    ...
    def rollout_state(self, state: CartpoleReplayState, *, train: bool) -> bool:
        ...
        for _ in range(int(self.args.max_steps)):
            ...
            if not self._is_alive(bsuite_state):
                return False
            if float(bsuite_state.time_elapsed) > self.max_time:
                return True
        return True

    def _is_alive(self, state) -> bool:
        return bool(np.cos(float(state.theta)) > self.height_threshold and abs(float(state.x)) < self.x_threshold)

    def teacher_action(self, state: CartpoleReplayState) -> int:
        vector = np.array([state.x, state.x_dot, centered_angle(state.theta), state.theta_dot], dtype=float)
        score = float(np.dot(TEACHER_WEIGHTS, vector))
...
def run_method(args: argparse.Namespace, method: str, seed: int) -> CartpoleProbeResult:
    ...
    bench = CartpoleRecoveryProbe(seed=seed, args=args)
    try:
...
def init_records(bench: CartpoleRecoveryProbe, rng: np.random.Generator, args: argparse.Namespace) -> list[LevelRecord]:
    ...
    for replay_idx, replay in enumerate(bench.states):
        record = LevelRecord(
            id=f"bsuite_cartpole_{replay_idx}",
            domain="bsuite_cartpole_recovery",
            task_id="bsuite-Cartpole",
            clean_success_hat=bench.success_prob(replay_idx, 0.0),
            feasibility_hat=1.0,
            perturbation_family="bounded_cartpole_state_restart",
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--out", default="runs/bsuite_cartpole_recovery_probe")
    ...
    parser.add_argument("--height-threshold", type=float, default=0.8)
    ...
    parser.add_argument("--teacher-stay-margin", type=float, default=0.05)
    parser.add_argument("--perturb-x-scale", type=float, default=1.20)
    parser.add_argument("--perturb-x-dot-scale", type=float, default=1.50)
    parser.add_argument("--perturb-theta-scale", type=float, default=0.50)
