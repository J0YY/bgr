#!/usr/bin/env python
"""Scout BGR-style replay on pre-existing sklearn tabular datasets.

This is an internal route-selection diagnostic, not paper evidence. It checks
whether built-in tabular datasets show a large enough fixed-protocol signal to
justify a preregistered benchmark route. The default gate is intentionally
strict: +0.03 mean RAUC over uniform and at least 3/4 paired wins.
"""
...
DEFAULT_DATASETS = ("breast_cancer", "wine")
DEFAULT_TARGETS = (0.5, 0.8, 1.0, 1.2, 1.5, 2.0)
...
def dataset_loaders() -> dict[str, Callable]:
    try:
...
def run_trial(
    *,
    ...
    return TrialResult(
        ...
        method=method,
        seed=seed,
        final_rauc=evaluate_rauc(model, x_eval[:250], y_eval[:250], eval_radii, rng),
    )
...
def main() -> int:
    ...
    parser.add_argument("--out", type=Path, default=Path("results/sklearn_tabular_margin_scout_v0"))
    ...
    unknown = sorted(set(args.datasets) - set(loaders))
    if unknown:
