...
class MountainCarRecoveryProbe:
            ...
            for pos_idx, position in enumerate(self.position_grid):
                for vel_idx, velocity in enumerate(self.velocity_grid):
                    ...
                    for action in range(3):
                        next_position, next_velocity, done = step_state(float(position), float(velocity), action)
    ...
    def loss_proxy(self, replay_idx: int, sigma: float, rng: np.random.Generator) -> float:
        ...
        next_position, next_velocity, done = step_state(position, velocity, action)
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Run an internal MountainCar-v0 recovery replay diagnostic.")
    parser.add_argument("--out", default="runs/mountaincar_recovery_probe")
