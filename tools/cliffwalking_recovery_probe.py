...
class CliffReplayState:
    row: int
    col: int


@dataclass(frozen=True, slots=True)
class CliffProbeResult:
    method: str
    seed: int
    final_clean: float
    final_rauc: float
...
def cliff_step(state: int, action: int) -> tuple[int, float, bool]:
    row, col = decode(state)
    dr, dc = ACTIONS[action]
    next_row = int(np.clip(row + dr, 0, ROWS - 1))
    next_col = int(np.clip(col + dc, 0, COLS - 1))
    if (next_row, next_col) == GOAL:
        return encode(next_row, next_col), -1.0, True
    if (next_row, next_col) in CLIFF:
        return encode(*START), -100.0, False
    return encode(next_row, next_col), -1.0, False
...
class CliffWalkingRecoveryProbe:
    def __init__(
        self,
        *,
        ...
        self.rng = np.random.default_rng(seed + 117_000)
    ...
    def _select_replay_states(self, count: int) -> list[CliffReplayState]:
        candidates = [
            CliffReplayState(row, col)
            for col in range(1, COLS - 1)
            for row in (2, 1)
            if (row, col) != GOAL
        ]
        candidates.sort(key=lambda item: (item.col, -item.row))
...
def run_method(args: argparse.Namespace, optimal_q: np.ndarray, method: str, seed: int) -> CliffProbeResult:
    rng = np.random.default_rng(seed + 119_000)
    bench = CliffWalkingRecoveryProbe(
        optimal_q=optimal_q,
        seed=seed,
...
def init_records(bench: CliffWalkingRecoveryProbe, rng: np.random.Generator, args: argparse.Namespace) -> list[LevelRecord]:
    records: list[LevelRecord] = []
    for replay_idx, replay in enumerate(bench.states):
        record = LevelRecord(
            id=f"cliffwalking_{replay.row}_{replay.col}",
            domain="cliffwalking_v0_recovery",
            task_id="CliffWalking-v0",
            clean_success_hat=bench.success_prob(replay_idx, 0.0),
            feasibility_hat=1.0,
            perturbation_family="grid_manhattan_restart",
...
def evaluate(bench: CliffWalkingRecoveryProbe, args: argparse.Namespace) -> dict[str, float]:
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Run an internal CliffWalking-v0 recovery replay diagnostic.")
    parser.add_argument("--out", default="runs/cliffwalking_recovery_probe")
    ...
    parser.add_argument("--replay-states", type=int, default=20)
