...
def package_versions() -> dict[str, str]:
    try:
        ...
        raise SystemExit(
            "MiniGrid DoorKey probe requires external minigrid/gymnasium packages. "
...
class OfficialMiniGridDoorKeyProbe:
    def __init__(
        self,
        *,
        seed: int,
        ...
        self.rng = np.random.default_rng(seed + 357_000)
        ...
        self.key_pos = self._find_object("key")
        self.door_pos = self._find_object("door")
        self.goal_pos = self._find_object("goal")
        self.key_color = self._object_color(self.key_pos)
        self.door_color = self._object_color(self.door_pos)
        self.state_to_idx, self.idx_to_state = self._build_state_index()
        self.transitions = self._build_transitions()
        self.distances = self._distances_to_success()
        ...
        if not self.states:
            raise ValueError("DoorKey probe selected no replay states")
        self.perturbation_cache = self._build_perturbation_cache()

    def _find_object(self, object_type: str) -> tuple[int, int]:
        for x in range(self.width):
            for y in range(self.height):
                obj = self.base_grid.get(x, y)
                if obj is not None and obj.type == object_type:
                    return int(x), int(y)
        raise ValueError(f"MiniGrid DoorKey layout has no {object_type!r}")

    def _object_color(self, pos: tuple[int, int]) -> str:
        obj = self.base_grid.get(*pos)
        if obj is None or not hasattr(obj, "color"):
            raise ValueError(f"object at {pos} has no color")
        return str(obj.color)

    def _static_wall(self, x: int, y: int) -> bool:
        obj = self.base_grid.get(x, y)
        return bool(obj is not None and obj.type == "wall")

    def _valid_agent_cells(self, *, has_key: bool, door_open: bool) -> list[tuple[int, int]]:
        cells: list[tuple[int, int]] = []
        for x in range(self.width):
            for y in range(self.height):
                if self._static_wall(x, y):
                    continue
                if (x, y) == self.door_pos and not door_open:
                    continue
    ...
    def _build_state_index(self) -> tuple[dict[DoorKeyState, int], dict[int, DoorKeyState]]:
        states: list[DoorKeyState] = []
        for has_key in (False, True):
            for door_open in (False, True):
                for x, y in self._valid_agent_cells(has_key=has_key, door_open=door_open):
                    for direction in DIRS:
                        states.append(DoorKeyState(x, y, direction, has_key, door_open))
        state_to_idx = {state: idx for idx, state in enumerate(states)}
        idx_to_state = {idx: state for state, idx in state_to_idx.items()}
        return state_to_idx, idx_to_state

    def _grid_for_status(self, *, has_key: bool, door_open: bool):
        from minigrid.core.world_object import Door, Key

        grid = self.base_grid.copy()
        grid.set(*self.key_pos, None if has_key else Key(self.key_color))
        grid.set(*self.door_pos, Door(self.door_color, is_open=door_open, is_locked=not door_open))
        return grid

    def _set_state(self, state_idx: int) -> None:
        from minigrid.core.world_object import Key

        state = self.idx_to_state[int(state_idx)]
        self.unwrapped.grid = self._grid_for_status(has_key=state.has_key, door_open=state.door_open)
        self.unwrapped.agent_pos = (int(state.x), int(state.y))
        self.unwrapped.agent_dir = int(state.direction)
        self.unwrapped.step_count = 0
        self.unwrapped.carrying = Key(self.key_color) if state.has_key else None

    def _state_from_env(self, fallback_idx: int) -> int:
        x, y = self.unwrapped.agent_pos
        direction = int(self.unwrapped.agent_dir)
        has_key = bool(self.unwrapped.carrying is not None and self.unwrapped.carrying.type == "key")
        door = self.unwrapped.grid.get(*self.door_pos)
        door_open = bool(getattr(door, "is_open", False))
        state = DoorKeyState(int(x), int(y), direction, has_key, door_open)
        return self.state_to_idx.get(state, int(fallback_idx))

    def _step_from(self, state_idx: int, action_idx: int) -> tuple[int, float, bool]:
        self._set_state(state_idx)
        _obs, _reward, terminated, truncated, _info = self.env.step(int(MOVE_ACTIONS[action_idx]))
        if bool(terminated):
            return state_idx, 1.0, True
        if bool(truncated):
            return state_idx, 0.0, True
        return self._state_from_env(state_idx), 0.0, False
    ...
    def _value_iteration(self, iterations: int = 1200) -> np.ndarray:
    ...
    def _distances_to_success(self) -> dict[int, int]:
        reverse: dict[int, list[int]] = {idx: [] for idx in range(len(self.state_to_idx))}
        goals: set[int] = set()
        for state_idx, row in enumerate(self.transitions):
            for next_idx, reward, done in row:
                if done and reward > 0.0:
                    goals.add(state_idx)
                else:
                    reverse[next_idx].append(state_idx)
        distances = {state: 0 for state in goals}
        queue = list(goals)
        for state in queue:
            for prev in reverse[state]:
                if prev not in distances:
                    distances[prev] = distances[state] + 1
                    queue.append(prev)
        return distances

    def _select_replay_states(self, count: int) -> list[DoorKeyReplayState]:
        door_x, door_y = self.door_pos
        candidates: list[tuple[int, int, int, DoorKeyReplayState]] = []
        for state, idx in self.state_to_idx.items():
            if not state.has_key or state.door_open:
                continue
            dist = self.distances.get(idx, self.width * self.height * 8)
            if self.replay_selection == "midband" and not (self.replay_distance_min <= dist <= self.replay_distance_max):
                continue
            door_dist = abs(state.x - door_x) + abs(state.y - door_y)
            if door_dist > 3:
                continue
            candidates.append((dist, door_dist, state.direction, DoorKeyReplayState(state.x, state.y, state.direction, dist)))
        if not candidates:
            for state, idx in self.state_to_idx.items():
                if state.has_key and not state.door_open:
                    dist = self.distances.get(idx, self.width * self.height * 8)
                    door_dist = abs(state.x - door_x) + abs(state.y - door_y)
                    candidates.append((dist, door_dist, state.direction, DoorKeyReplayState(state.x, state.y, state.direction, dist)))
        candidates.sort(key=lambda item: (item[0], item[1], item[2], item[3].x, item[3].y))
        if self.replay_selection == "doorward":
    ...
    def _build_perturbation_cache(self) -> tuple[tuple[tuple[int, ...], ...], ...]:
        ...
        valid_cells = self._valid_agent_cells(has_key=True, door_open=False)
        for replay in self.states:
            radii: list[tuple[int, ...]] = []
            for radius in range(self.max_radius + 1):
                states = tuple(
                    self.state_to_idx[DoorKeyState(x, y, replay.direction, True, False)]
                    for x, y in valid_cells
                    if abs(x - replay.x) + abs(y - replay.y) == radius
                )
                radii.append(states or (self.state_to_idx[DoorKeyState(replay.x, replay.y, replay.direction, True, False)],))
    ...
    def current_success_values(self) -> np.ndarray:
        ...
        return np.array(
            [float(self._rollout_state(state_idx, policy=policy, train=False, epsilon=False)) for state_idx in range(len(self.state_to_idx))],
            dtype=float,
        )
...
def run_method(args: argparse.Namespace, method: str, seed: int) -> DoorKeyProbeResult:
    rng = np.random.default_rng(seed + 359_000)
    bench = OfficialMiniGridDoorKeyProbe(
        seed=seed,
        env_id=args.env_id,
...
def init_records(bench: OfficialMiniGridDoorKeyProbe, rng: np.random.Generator, args: argparse.Namespace) -> list[LevelRecord]:
    records: list[LevelRecord] = []
    for replay_idx, replay in enumerate(bench.states):
        record = LevelRecord(
            id=f"minigrid_doorkey_{replay.x}_{replay.y}_{replay.direction}",
            domain="minigrid_doorkey_recovery",
            task_id=bench.env_id,
            clean_success_hat=bench.success_prob(replay_idx, 0.0),
            feasibility_hat=1.0,
            perturbation_family="official_minigrid_doorkey_position_restart_key_carried_door_closed",
...
def evaluate(bench: OfficialMiniGridDoorKeyProbe, args: argparse.Namespace) -> dict[str, float]:
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Run an internal official MiniGrid-DoorKey recovery replay diagnostic.")
    parser.add_argument("--out", default="runs/minigrid_doorkey_recovery_probe")
    ...
    parser.add_argument("--replay-selection", choices=["spread", "doorward", "midband"], default="midband")
    parser.add_argument("--replay-distance-min", type=int, default=3)
    parser.add_argument("--replay-distance-max", type=int, default=10)
    ...
    parser.add_argument("--q-init-blend", type=float, default=0.04)
