...
REMOTE_VENV="${REMOTE_VENV:-${REMOTE_PROJECT}/.venv-swimmer-mujoco}"
OUT_PREFIX="${OUT_PREFIX:-swimmer_recovery_calibration_12seed_v1}"
...
rsync -az tools/swimmer_recovery_calibration.py "${REMOTE_HOST}:${REMOTE_PROJECT}/tools/"
...
ssh "${REMOTE_HOST}" "cd '${REMOTE_PROJECT}' && \
  ...
  '${REMOTE_VENV}/bin/python' -m pip install 'numpy==2.2.6' 'gymnasium[mujoco]==1.3.0'"
...
PYTHONPATH='${REMOTE_PROJECT}/src:${REMOTE_PROJECT}' \
  ...
  '${REMOTE_PROJECT}/tools/swimmer_recovery_calibration.py' \
...
printf 'submitted Swimmer calibration job: %s\n' "${job_id}"
printf 'remote output: %s/%s_%s\n' "${REMOTE_RUN_ROOT}" "${OUT_PREFIX}" "${job_id}"
printf 'remote log: %s/bgr-swimmer-calib-%s.out\n' "${REMOTE_LOG_ROOT}" "${job_id}"
