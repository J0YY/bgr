...
def main() -> None:
    ...
    parser.add_argument("--config", default="configs/grid_margin_stress_sensitivity_15seed.yaml")
    parser.add_argument("--out", default="runs/grid_margin_stress_sensitivity")
    ...
    for case in cases:
        for method in methods:
            for seed in sweep_config["experiment"]["seeds"]:
                ...
                result_payload.update(case.to_row())
...
def parse_stress_cases(config: dict) -> list[StressCase]:
    ...
    for raw in raw_cases:
        if not isinstance(raw, dict):
            raise ValueError("each stress case must be a mapping")
        if "name" not in raw:
            raise ValueError("each stress case requires a name")
        overrides = {str(key): value for key, value in raw.items() if key != "name"}
        if not overrides:
            raise ValueError(f"stress case {raw['name']!r} has no overrides")
        cases.append(StressCase(str(raw["name"]), overrides))
    return cases


def config_for_stress_case(base_config: dict, case: StressCase, seed: int, method: str) -> dict:
    config = copy.deepcopy(base_config)
    config["experiment"]["name"] = f"{base_config['experiment'].get('name', 'grid_margin')}_{case.name}"
...
def _summary(rows: list[dict[str, float | str | int]]) -> str:
    ...
    lines = ["stress_case,method,n,final_clean_mean,final_rauc_mean,final_median_r80_mean,rauc_aulc_mean,best_rauc_mean"]
