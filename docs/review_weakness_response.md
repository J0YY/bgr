...
| Proposition too strong | Proposition is renamed local boundary intuition and explicitly says it is not a convergence, global robustness, or margin-expansion theorem. The method section also adds a finite-grid estimator guarantee: with fixed Bernoulli probes, the monotone recovery estimate has a Hoeffding/union-bound error and the critical-radius estimate is controlled by grid spacing, local slope, and threshold error. | This still does not prove global learner improvement; it only strengthens the estimator piece. | Medium |
